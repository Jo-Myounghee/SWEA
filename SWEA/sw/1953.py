...
def main():
    ...
    def bfs(x, y):
        ...
        while q:
            ...
            if cnt == L:
                return total
            pipe_type = board[ny][nx]
            for d in dir_lst[pipe_type]:
                tx, ty = nx + dx[d], ny + dy[d]
                if 0 <= tx < M and 0 <= ty < N and not visited[ty][tx] and board[ty][tx] > 0:
                    if is_connected(d, board[ty][tx]):
